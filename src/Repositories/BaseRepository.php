...
abstract class BaseRepository implements RepositoryContract, CacheableContract
{
    ...
    /**
     * Additional data to be used when generating a cache key.
     *
     * This is useful for complex joins.
     */
    protected $cacheData = [ ];
    ...
    protected function prepareQuery($model)
    {
        ...
        foreach ($this->join as $join) {
            ...
            $model = $model->join($foreignTable, $primaryKey, $operator, $foreignKey);
        }
    ...
    /**
     * Add additional data for caching key generation purposes. This is useful for complex
     * joins.
     *
     * @param string $name
     * @param mixed $data
     */
    protected function addCacheData( $name, $data )
    {
        if ( is_array( $data ) ) {
            $this->cacheData[ $name ] = array_sort( $data );
        } else {
            $this->cacheData[ $name ] = $data;
        }
        ksort( $this->cacheData );
    }
}
